...
def run_knn():
    ...
    class_rates = []
    for k in ks:
        predictions = knn(k, train_inputs, train_targets, valid_inputs)
        accuracy_vec = valid_targets == predictions
        class_rates.append(np.count_nonzero(accuracy_vec) /
                           np.size(accuracy_vec))
    print(class_rates)
    plt.title('Classification Rates for Different k')
    plt.xlabel('k')
    plt.ylabel('Classification Rate')
    plt.plot(ks, class_rates)
    plt.show()

    for k in [3, 5, 7]:
        predictions = knn(k, train_inputs, train_targets, test_inputs)
        accuracy_vec = test_targets == predictions
        print(np.count_nonzero(accuracy_vec) / np.size(accuracy_vec))
