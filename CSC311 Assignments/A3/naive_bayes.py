...
def load_mnist():
    partial_flatten = lambda x: np.reshape(x, (x.shape[0],
                                               np.prod(x.shape[1:])))
...
def train_mle_estimator(train_images, train_labels):
    ...
    for feature in train_images.T:
        counts = feature.dot(train_labels)
        theta_mle.append(counts / totals)

    pi_mle = totals / np.size(train_images)
    return np.array(theta_mle), pi_mle
...
def train_map_estimator(train_images, train_labels):
    ...
    for feature in train_images.T:
        counts = feature.dot(train_labels)
        theta_map.append((counts+2) / (totals+4))

    pi_map = totals / np.size(train_images)
    return np.array(theta_map), pi_map
...
def log_likelihood(images, theta, pi):
    ...
    log_like = []
    for x in images:
        prior = np.log(pi) + x.dot(np.log(theta)) + (1-x).dot(np.log(1-theta))
        evidence = np.log(np.sum(np.exp(prior)))
        log_like.append(prior - evidence)
...
def accuracy(log_like, labels):
    ...
    return np.count_nonzero(labels.nonzero()[1] == predictions) /\
        predictions.size
...
def image_sampler(theta, pi, num_images):
    ...
    for _ in range(num_images):
        c = np.random.choice([i for i in range(10)])
        sampled_images.append(np.random.binomial(1, theta[:, c]))
