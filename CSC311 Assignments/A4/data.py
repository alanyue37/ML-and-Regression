# Authors of this file: University of Toronto CSC311 Course Team
...
def load_data(data_dir, stem):
    """
    Loads data from either the training set or the test set and returns the
    pixel values and class labels
    """
    data = []
    labels = []
    for i in range(0, 10):
...
def load_all_data(data_dir, shuffle=True):
    ...
    if not os.path.isdir(data_dir):
        raise OSError('Data directory {} does not exist. Try'
                      '"load_all_data_from_zip" function first.'
                      .format(data_dir))
    ...
    if shuffle:
        ...
        train_data, train_labels = train_data[train_indices],\
            train_labels[train_indices]
        test_data, test_labels = test_data[test_indices],\
